...
`plot_step_ahead_forecasts()` that can be used to plot model output that is 
...
As all forecasts in forecast_data will be plotted, any filtering needs to 
...
```
  ...
  - By default, separate colors will be used for each model. It is possible to use only blues for all models, by setting the `fill_by_model` parameter to `FALSE`. This might be especially useful when used for many models in conjunction with highlighting the ensemble forecast using the `ens_name` and `ens_color` argument.
```{r plot_us_mod}
plot_step_ahead_forecasts(projection_data_A_us, truth_data_us, 
                          intervals = 0.8,
                          ens_name = "hub-ensemble", ens_color = "black",
