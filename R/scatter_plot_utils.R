...
plotly_truth_data <- function(plot_model, truth_data, plot_truth, show_legend,
  ...
  if (plot_truth) {
    ...
    arg_list <- list(p = plot_model, data = truth_data,
                     x = truth_data[[x_col_name]], y = ~value, type = "scatter",
                     mode = "lines+markers", line = list(color = "#6e6e6e"),
                     hoverinfo = "text", name = "ground truth",
                     legendgroup = "ground truth",
                     hovertext = paste("Date: ", truth_data[[x_col_name]],
                                       "<br>Ground truth: ",
                                       format(truth_data$value, big.mark = ","),
                                       sep = ""),
