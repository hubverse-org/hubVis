...
plot_prep_data <- function(
  ...
  if (
    length(quant) > 0 &&
      !("quantile" %in% df$output_type) &&
      ("sample" %in% df$output_type)
  ) {
    df_sample <- dplyr::filter(df, .data[["output_type"]] == "sample")
    cli::cli_inform(c(
      "i" = "{.val sample} output_type is used to calculate
                      required {.val quantile}"
    ))
    quant_df <- hubUtils::convert_output_type(
      df_sample,
  ...
  if (any(is_empty_col)) {
    ...
    cli::cli_warn(c(
      "!" = "{.arg model_out_tbl} contains some empty
      ...
      ribbon_df <-
        transform(
          ...
          output_type_id = ifelse(
            dplyr::near(ribbon_df$output_type_id, min(ribbon)),
            "min",
            "max"
          )
        )
      id_col <- colnames(ribbon_df)[
        !colnames(ribbon_df) %in%
          c("output_type_id", "value")
      ]
      ribbon_df <- reshape(
        ribbon_df,
        timevar = "output_type_id",
        direction = "wide",
        idvar = id_col
      )
...
plotly_target_data <- function(
  ...
  if (plot_target) {
    ...
    arg_list <- list(
      p = plot_model,
      data = target_data,
      x = target_data[[x_col_name]],
      y = ~observation,
      type = "scatter",
      mode = "lines+markers",
      line = list(color = "#6e6e6e"),
      hoverinfo = "text",
      name = "target",
      legendgroup = "target",
      hovertext = paste(
        "Date: ",
        target_data[[x_col_name]],
        "<br>target: ",
        format(target_data$observation, big.mark = ","),
        sep = ""
      ),
...
plotly_proj_data <- function(
    ...
    arg_list <-
      list(
        ...
        hovertext = paste(
          ...
          "<br>",
          "Median: ",
          format(round(df_point$value, 2), big.mark = ","),
          sep = ""
        )
      )
  ...
  if (!is.null(df_sample)) {
    for (j in unique(df_sample[[fill_by]])) {
      ...
      if (!is.null(group)) {
        df_sample_id <- dplyr::group_by(
          df_sample_id,
          .data[[group]],
          .data[["output_type_id"]]
        )
      } else {
        df_sample_id <- dplyr::group_by(df_sample_id, .data[["output_type_id"]])
      }
      arg_list <-
        list(
          p = plot_model,
          data = df_sample_id,
          x = df_sample_id[[x_col_name]],
          y = df_sample_id[["value"]],
          type = "scatter",
          mode = "lines",
          showlegend = show_leg,
          opacity = opacity,
          name = df_sample_id[[fill_by]],
          legendgroup = df_sample_id[[fill_by]]
        )
      if (is.null(line_color)) {
        arg_list <- c(
          arg_list,
          list(color = df_sample_id[[fill_by]]),
          arguments
        )
      } else {
        arg_list <- c(
          arg_list,
          list(line = list(color = line_color)),
          arguments
        )
      }
      plot_model <- do.call(plotly::add_trace, arg_list)
    ...
    for (n_rib in seq_along(df_ribbon)) {
      ...
      if (nrow(df_rib) > 0) {
        ...
        if (is.null(line_color)) {
          arg_list <- c(
            arg_list,
            list(color = df_rib[[fill_by]], line = list(width = 0)),
            arguments
          )
...
static_proj_data <- function(
      ...
      plot_model <- plot_model +
        geom_line(
          ...
          aes(
            ...
            y = .data$value,
            color = .data[[fill_by]],
            group = paste0(.data[[group]], "-", .data[["output_type_id"]])
          ),
          alpha = opacity
        )
    }
  ...
  if (nrow(df_point) > 0) {
    ...
    if (is.null(group)) {
      ...
      df_point$group <- as.factor(paste0(
        df_point[[fill_by]],
        df_point[[group]]
      ))
      plot_aes <- aes(
...
plot_model_layout_attr <- function(plot_model, attribute_name, attributes) {
  ...
  attr_update <- purrr::map(attr_name, function(x) {
    setNames(list(attributes), x)
  })
...
plot_layout <- function(
  ...
  if (interactive) {
    ...
    if (!is.null(facet)) {
      ...
      if (log_scale) {
        plot_model <- plot_model_layout_attr(
          plot_model,
          "yaxis",
          list(type = "log")
        )
      ...
      plot_model <- plotly::layout(plot_model, yaxis = list(title = "Value"))
      if (log_scale) {
