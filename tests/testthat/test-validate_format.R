...
test_that("Input parameters", {
  ...
  d_proj <- rbind(
    projection_data_a_us,
    dplyr::mutate(projection_data_a_us, model_id = paste0(model_id, "_2"))
  )
  ...
  err_mess <- paste0(
    "did not have the expected output_type_id value 0.975, ",
  ...
  p <- plot_step_ahead_model_output(
    ...
    suppressWarnings()
  expect_equal(
    grep("Median", p$x$attrs[[p$x$cur_data]]$hovertext) |> length(),
    dplyr::filter(
      forecast_quantile,
  ...
  expect_warning(
    ...
    paste0(
      "output_type will be used to calculate the quantiles",
  ...
  expect_message(
    plot_step_ahead_model_output(
      dplyr::filter(forecast_data, output_type == "sample"),
      target_data_us,
      use_median_as_point = TRUE,
      x_col_name = "target_end_date",
      intervals = 0.5
  ...
  expect_equal(
    grep("Median", unlist(purrr::map(p$x$attrs, "hovertext"))) |>
      length(),
  ...
  expect_equal(
    grep(
      "Intervals",
      unlist(purrr::map(p$x$attrs, "hovertext")),
      value = TRUE
    ) |>
      length(),
    grep(
