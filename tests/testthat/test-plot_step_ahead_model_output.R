...
test_that("Output", {
  ...
  expect_in(
    names(tail(plot_test$layers, 1)[[1]]$data),
  ...
  attr_test <- purrr::map(c(3:length(plot_test$x$visdat)), function(x) {
    attributes(plot_test$x$visdat[[x]]())
  })
  ...
  expect_equal(
    unique(attr_test)[[1]]$forecast_date,
  ...
  expect_equal(
    purrr::map(purrr::map(plot_test$x$layoutAttrs, "yaxis"), "type") |>
      unlist() |>
      unique(),
    "log"
  )
  ...
  test_value <-
    purrr::map(
      plot_test$x$data[
        as.character(unlist(purrr::map(plot_test$x$data, "name"))) ==
          "hub-ensemble"
      ],
      "line"
    )
  ...
  plot_test <-
    plot_step_ahead_model_output(
      proj_data_q,
      dplyr::mutate(target_data_us, scenario_id = "A-2021-03-05"),
      facet = "scenario_id",
      plot_target = TRUE,
      facet_scales = "free",
      show_legend = FALSE,
  ...
  test_value <-
    purrr::map(
      plot_test$x$data[purrr::map(plot_test$x$data, "name") == "target"],
      "x"
    )
  ...
  expect_in(
    unique(unlist(purrr::map(purrr::map(plot_test$x$attrs, "color"), levels))),
  ...
  test_value <-
    purrr::map(
      plot_test$x$data[
        purrr::map(purrr::map(plot_test$x$data, "name"), levels) %in%
          "hub-ensemble"
      ],
      "line"
    )
  ...
  test_value <-
    purrr::map(
      plot_test$x$data[
        purrr::map(purrr::map(plot_test$x$data, "name"), as.character) ==
          "HUBuni-simexamp"
      ],
      "line"
    )
  ...
  expect_equal(
    strsplit(
      strsplit(
        tail(purrr::map(plot_test$x$attrs, "hovertext"), 1)[[1]],
        "<br>"
      )[[1]][2],
      "Intervals"
    )[[1]][1],
